...
final class FSAlbumView: UIView, UICollectionViewDataSource, UICollectionViewDelegate, PHPhotoLibraryChangeObserver, UIGestureRecognizerDelegate {
    ...
    func initialize() {
        ...
        if images.count > 0 {
            ...
            collectionView.selectItem(at: IndexPath(row: index, section: 0), animated: false, scrollPosition: UICollectionView.ScrollPosition())
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let asset = self.images[self.images.count - indexPath.row - 1]
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let reversedIndex = images.count - (indexPath as NSIndexPath).row - 1
        changeImage(images[reversedIndex])
    ...
    func photoLibraryDidChange(_ changeInstance: PHChange) {
        ...
        DispatchQueue.main.async {
            if let collectionChanges = changeInstance.changeDetails(for: self.images), let collectionView = self.collectionView {
